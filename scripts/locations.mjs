  ...
  const faqs = [
    {
      ...
      a: vpick(vb, "fa1", [
        ...
        `${gu.name} ${dongName} 기준으로 위치와 희망 시간을 전화로 전달하면 방문 가능 여부를 확인할 수 있습니다. 같은 ${gu.name} 안에서도 권역에 따라 도착 시간이 달라질 수 있습니다.`,
      ]),
    },
    {
      q: `${dongName}에서 홈타이도 이용할 수 있나요?`,
      a: vpick(vb, "fa2", [
        `홈타이는 집·숙소로 받는 방문 형태의 출장마사지를 뜻하며, 서울 ${gu.name} ${dongName}에서도 방문 가능 권역인지 예약 시 확인하면 됩니다. 스웨디시·아로마·타이마사지 등 프로그램을 선택할 수 있습니다.`,
        `네, 홈타이는 ${gu.name} ${dongName} 인근 집·숙소로 받는 방문 관리입니다. 권역에 포함되는지 예약 시 확인하고 원하는 프로그램·시간을 함께 정하면 됩니다.`,
        `${dongName}을(를) 포함한 서울 ${gu.name} 권역에서 홈타이 방문이 가능한지는 예약 시 위치로 확인됩니다. 집·숙소 어디서 받을지와 프로그램을 함께 알려 두면 안내가 빠릅니다.`,
      ]),
    },
    {
      q: `${dongName}까지 방문에 얼마나 걸리나요?`,
      a: vpick(vb, "fa3", [
        `출발지와 시간대, 서울 ${gu.name} 내 권역에 따라 달라집니다. 정확한 방문 소요 시간은 예약 시 확인하는 것이 좋습니다.`,
        `${gu.name} ${dongName}은(는) 출발지와 교통 상황에 따라 도착 시간이 달라집니다. 예약 시 위치를 알리면 예상 소요 시간을 안내받을 수 있습니다.`,
        `서울 ${gu.name} 안에서도 ${dongName} 권역과 출발지에 따라 도착 시간이 다릅니다. 희망 시간대를 미리 정해 두면 안내가 정확합니다.`,
      ]),
    },
  ];
