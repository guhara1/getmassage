...
function stationPage(reg, sys) {
  ...
  const faqs = [
    ...
    { q: `${station}에서 홈타이도 이용할 수 있나요?`, a: vpick(vb, "fa2", [
      `홈타이는 집·숙소로 받는 방문 형태의 출장마사지를 뜻하며, ${sysName} ${l1} ${station} 인근에서도 방문 가능 권역인지 예약 시 확인하면 됩니다. 스웨디시·아로마·타이마사지 등 프로그램을 선택할 수 있습니다.`,
      `네, 홈타이는 ${sysName} ${station} 인근 집·숙소로 받는 방문 관리입니다. 권역에 포함되는지 예약 시 확인하고, 원하는 프로그램과 시간을 함께 정하면 됩니다.`,
      `${station}(${sysName} ${l1}) 인근 집·숙소도 홈타이 방문이 되는지는 예약 시 위치로 확인됩니다. 받을 장소와 프로그램을 함께 알려 두면 안내가 빠릅니다.`,
    ]) },
    { q: `${station}까지 방문에 얼마나 걸리나요?`, a: vpick(vb, "fa3", [
      `출발지와 시간대, ${sysName} ${station} 역 인근 권역에 따라 달라집니다. 정확한 방문 소요 시간은 예약 시 확인하는 것이 좋습니다.`,
      `${where}에 있는 ${sysName} ${station}은(는) 출발지와 교통 상황에 따라 도착 시간이 달라집니다. 예약 시 위치를 알리면 예상 소요 시간을 안내받을 수 있습니다.`,
      `${sysName} ${l1} ${station} 권역은 출발지에 따라 도착 시간 차이가 있습니다. 희망 시간대와 위치를 함께 전달하면 예상 소요 시간을 안내받을 수 있습니다.`,
    ]) },
  ];
  ...
  // ── 접근·주변 동선 (역별 고유 정보로 본문 보강) ──
  const secAccess = `
    <h2>${esc(vpick(vb, "hAccess", [
      `${station} 주변 접근과 이동 동선`,
      `${station} 어떻게 찾아가나요`,
      `${station} 인근 접근성 안내`,
      `${station} 주변 동선 살펴보기`,
    ]))}</h2>
    <p>${esc(vpick(vb, "accA", [
      `${station}은(는) ${lineText} 동선 위에 있어 ${neighText} 방면에서 넘어오거나 ${n1} 쪽에서 이동해 오는 경우가 많습니다. ${isTransfer ? "환승역이라 여러 방향에서 접근이 쉬운 만큼, 출발지를 미리 알려 두면 도착 시간 안내가 정확합니다." : "단일 노선 역이라 동선이 단순한 편이어서, 가까운 출입구를 기준으로 위치를 알리면 안내가 수월합니다."}`,
      `방문 관리는 매장을 찾아가는 방식이 아니라 ${station} 인근 집·숙소로 관리사가 이동해 오는 형태라, 역 자체보다 실제 머무는 위치(동·건물)가 도착 시간을 좌우합니다. 같은 ${station} 권역이라도 ${neighText} 쪽이면 동선이 달라질 수 있습니다.`,
      `${station} 일대는 ${where}에 자리해 ${sysName} 내 다른 권역과 도착 시간 차이가 생길 수 있습니다. ${n1} 등 인접역과 묶어 권역을 살피면 어느 출발지가 가까운지 가늠하기 쉽습니다.`,
      `${station}에서 이용할 때는 큰 길이나 랜드마크보다 머무는 곳의 정확한 주소가 더 중요합니다. ${lineText} 이용 동선과 함께 위치를 전달하면 방문 경로와 예상 소요 시간을 안내받을 수 있습니다.`,
    ]))}</p>
    <p>${esc(vpick(vb, "accB", [
      `오피스텔·숙소에서 받는 경우에는 출입 방법(공동현관 비밀번호, 프런트 경유 등)과 주차 가능 여부를 예약 시 함께 알려 두면 방문이 매끄럽습니다.`,
      `건물에 따라 출입 절차가 다를 수 있어, ${station} 인근 숙소·오피스텔이라면 공동현관·엘리베이터·주차 안내를 미리 정리해 두는 것이 좋습니다.`,
      `방문 동선을 줄이려면 정확한 동·호수와 출입 방법을 예약 단계에서 전달하는 것이 가장 확실합니다. 늦은 시간이라면 야간 출입 가능 여부도 함께 확인하세요.`,
    ]))}</p>`;

  // ── 시간대·상황별 이용 안내 (역별 고유 정보로 본문 보강) ──
  const secTiming = `
    <h2>${esc(vpick(vb, "hTiming", [
      `${station} 시간대별 이용 안내`,
      `${station} 언제 이용하면 좋을까`,
      `${station} 상황별 이용 팁`,
      `${station} 이용 시간 고르기`,
    ...
    <p>${esc(vpick(vb, "timA", [
      `낮 시간대는 ${station} 인근 이동이 비교적 여유로워 예약·도착 안내가 빠른 편이고, 저녁·심야로 갈수록 문의가 몰려 도착까지 시간이 더 걸릴 수 있습니다. 원하는 시간이 분명하다면 조금 일찍 문의해 두는 편이 안전합니다.`,
      `퇴근 직후 시간대는 ${station} 같은 역세권에 문의가 집중되는 경우가 많습니다. 여유 있게 받고 싶다면 피크 시간을 살짝 피하거나 미리 예약 시간을 잡아 두면 대기 없이 진행하기 쉽습니다.`,
      `${station}에서 심야 이용을 생각한다면 심야 방문 가능 여부와 추가 요금, 도착 소요 시간을 예약 단계에서 함께 확인해 두는 것이 좋습니다. 시간대에 따라 운영 조건이 달라질 수 있습니다.`,
      `주말·공휴일은 평일과 예약 흐름이 다를 수 있어, ${station} 인근에서 원하는 시간대가 있다면 미리 문의해 두는 편이 여유롭습니다. 코스 길이에 따라 종료 시간도 함께 고려하면 일정 잡기가 수월합니다.`,
    ]))}</p>
    <p>${esc(vpick(vb, "timB", [
      `어떤 시간대든 표시된 운영 시간과 요금은 변동될 수 있으므로, 방문 가능 여부와 총 비용은 예약 시점에 ${phone}로 확인하는 것이 정확합니다.`,
      `시간대와 관계없이 위치·프로그램·희망 시간을 함께 전달하면 도착 예정 시간을 안내받을 수 있습니다. 정확한 비용은 예약 시 ${phone}로 확인하세요.`,
      `이용 시간대가 정해지면 코스(60·90·120분)와 마무리 예정 시간을 함께 가늠해 두면 다음 일정과 겹치지 않게 잡을 수 있습니다.`,
    ]))}</p>`;

  const middle = vshuffle(vb, "order", [secWho, secCheck, secCompare, secFlow, secTips, secAccess, secTiming]).join("\n");
...
function linePage(line, sys) {
  ...
  const faqs = [
    ...
    { q: vpick(vb, "fq3", [
        `${line.name} 예약은 어떻게 하나요?`,
        `${line.name}에서 예약은 어떻게 진행하나요?`,
        `예약은 어떤 순서로 하나요?`,
      ]), a: vpick(vb, "fa3", [
        `전화로 ${line.name}에서 원하는 역과 프로그램·시간을 알리면 방문 가능 여부와 도착 소요 시간을 안내받을 수 있습니다. 전화예약 ${phone}.`,
        `${line.name} 가까운 역을 고른 뒤 위치·프로그램·시간을 전화로 전달하면 도착 예정 시간과 비용을 안내받습니다. 전화예약 ${phone}.`,
        `${first}·${last} 등 ${line.name} 역 중 원하는 곳을 정하고 프로그램·시간을 알리면 방문 가능 여부를 확인할 수 있습니다. 전화예약 ${phone}.`,
      ]) },
  ];
